...
import {loadWhitelistClubList} from 'redux/reducer/club'
import {clubListSchema} from 'redux/schema/index'
import {withTranslate} from 'hocs/index'
import {UserGroupIcon} from '@heroicons/react/outline'

import {defaultListData} from 'helper/common'
import MustConnectWalletWrapper from 'components/must_connect';
...
class MyClubList extends React.Component {
    ...
    render() {
        ...
        return <MustConnectWalletWrapper>
        <div>
            {
...
function mapStateToProps(state,ownProps) {
    ...
    let list_data = state.getIn(['club','whitelist_list'])
...
const mapDispatchToProps = (dispatch) => {
    return {
        loadList   : (cond) => {
            return dispatch(loadWhitelistClubList(cond))
        },
    }
}
